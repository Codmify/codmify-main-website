              ...
              <Grid
                ...
                <Stack
                  ...
                  sx={{
                    ...
                    position: "relative",
                    overflow: "hidden",
                    boxShadow: pkg.featured ? "0 12px 30px rgba(18,18,121,.16)" : "0 8px 24px rgba(22, 64, 98, .06)",
                    transition: "transform .25s ease, box-shadow .25s ease",
                    "&::before": { content: '""', position: "absolute", width: 160, height: 160, borderRadius: "50%", top: -90, right: -70, background: pkg.featured ? "rgba(0, 141, 229, .12)" : "rgba(0, 141, 229, .07)" },
                    "&::after": { content: '""', position: "absolute", width: 85, height: 85, borderRadius: "50%", bottom: -45, left: -35, border: "18px solid rgba(18, 18, 121, .04)" },
                    "& > *": { position: "relative", zIndex: 1 },
                    "&:hover": { transform: "translateY(-6px)", boxShadow: "0 16px 32px rgba(18, 18, 121, .14)" }
                  }}>
                  ...
                  <Box>
                    ...
                    <Typography
                      ...
                      }}>{pkg.audience}</Typography>
                  </Box>
                  <Box>
                    {!pkg.customQuote && <Typography
                      variant="caption"
                      sx={{
                        fontWeight: 700,
                        color: "#4A5E6D"
                      }}>STARTING FROM</Typography>}
                    <Typography
                      sx={{
                        ...
                        fontWeight: 700,
                        color: "#008DE5"
                      }}>{displayPrice}</Typography>
                  </Box>
                  <Typography variant="body2" sx={{
                    color: "#4A5E6D"
                  }}>{pkg.intro}</Typography>
                  <Stack spacing={1} sx={{
                    flexGrow: 1
                  }}>
                    {pkg.features.map((feature) => <Stack key={feature} direction="row" spacing={0.8} sx={{
                      alignItems: "flex-start"
                    }}><IoCheckmarkCircle color="#008DE5" size={18} style={{ marginTop: 2, flexShrink: 0 }} /><Typography variant="body2">{feature}</Typography></Stack>)}
                  </Stack>
                  <Box
                    sx={{
                      pt: 1,
