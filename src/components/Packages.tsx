...
import { getPackageWhatsAppUrl, lowestStartingPriceNGN, websitePackages } from "@/utils/packages";
      ...
      <Container maxWidth="lg">
        <Stack alignItems="center" textAlign="center" spacing={1.5} mb={{ xs: 4, md: 6 }}>
          <Typography variant="h3" fontWeight={700} color="#121279" fontSize={{ xs: 30, md: 42 }}>
            Smart websites from {formatPrice(lowestStartingPriceNGN, currency)}
          </Typography>
        ...
        <Grid container spacing={3} alignItems="stretch">
          {websitePackages.map((pkg) => {
            const displayPrice = pkg.startingPriceNGN !== undefined ? formatPrice(pkg.startingPriceNGN, currency) : pkg.customQuote ?? "";
            return (
