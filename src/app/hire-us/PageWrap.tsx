  ...
  return (
    <LandingPage>
          ...
          <Grid
            ...
            <Typography
              ...
              sx={{
                ...
                lineHeight: 1.06,
                fontWeight: 700
              }}>Tell us what you want to build.</Typography>
            <Typography
              sx={{
                fontSize: { xs: 17, md: 19 },
                color: "rgba(255,255,255,.82)"
              }}>Share a few details and our team will help you turn your idea into a clear, practical next step.</Typography>
          </Stack></Grid>
          <Grid
            size={{
              xs: 12,
            ...
            {["Clear recommendations tailored to your goals", "A straightforward delivery plan and timeline", "Response from our team within 1–2 business days"].map((item) => <Stack key={item} direction="row" spacing={1.25} sx={{
              ...
              sx={{
                ...
                fontSize: 20,
                mt: "2px"
              }} /><Typography>{item}</Typography></Stack>)}
          </Stack></Grid>
        </Grid></Container>
      </Box>
      ...
      <Box
        sx={{
          py: { xs: 6, md: 10 },
          backgroundImage: "radial-gradient(circle at 10% 4%, rgba(0,141,229,.1), transparent 23rem), radial-gradient(circle at 95% 70%, rgba(18,18,121,.06), transparent 22rem)"
        }}>
        <Container maxWidth="lg"><Box
          ...
          <Stack spacing={1} sx={{
            ...
            sx={{
              ...
              fontWeight: 700,
              letterSpacing: 1.2
            }}>PROJECT ENQUIRY</Typography><Typography
            component="h2"
            sx={{
              fontSize: { xs: 30, md: 42 },
              color: "#121279",
              fontWeight: 700
            }}>A few details to get started.</Typography><Typography sx={{
            color: "#526573"
          }}>Select the areas where you need support, then tell us a little about your project.</Typography></Stack>
          <Stack
            spacing={4}
            sx={{
              p: { xs: 2, md: 4 },
              ...
              borderRadius: 4,
              boxShadow: "0 14px 36px rgba(18,18,121,.08)"
            }}>
            <Box><Stack
              direction="row"
              sx={{
              ...
              sx={{
                color: "#121279",
                fontWeight: 700
              }}>1. What can we help with?</Typography><Typography variant="body2" sx={{
              color: "#526573"
            }}>Choose one or more services.</Typography></Box><Button disabled={loading} onClick={() => setSelected(selected.length === servicesTitles.length ? [] : servicesTitles)} sx={{ color: "#121279", whiteSpace: "nowrap" }}>{selected.length === servicesTitles.length ? "Clear all" : "Select all"}</Button></Stack>
              <Stack
                direction="row"
                sx={{
                  flexWrap: "wrap",
                  gap: 1.25
                }}>{servicesHolder.map((service) => { const active = selected.includes(service.title); return <Button key={service.title} type="button" onClick={() => handleClick(service.title)} variant={active ? "contained" : "outlined"} disabled={loading} startIcon={active ? <FiCheck /> : undefined} sx={{ borderColor: active ? "#121279" : "#D7E2EB", bgcolor: active ? "#121279" : "white", color: active ? "white" : "#334957", px: 1.8, "&:hover": { borderColor: "#121279", bgcolor: active ? "#121279" : "#F1F6FA" } }}>{service.title}</Button>; })}</Stack>
            </Box>
            <Box><Typography
              variant="h6"
              sx={{
              ...
              <Grid container spacing={2.5}>
                {[{ label: "Your name", name: "name", placeholder: "e.g. John Doe", required: true }, { label: "Email address", name: "email", placeholder: "you@company.com", required: true, type: "email" }, { label: "Phone number", name: "phone", placeholder: "e.g. +234 800 000 0000", required: true, type: "tel" }, { label: "Company name", name: "companyName", placeholder: "Optional" }].map((field) => <Grid
                  ...
                  sx={{
                    ...
                    mb: 0.75,
                    display: "block"
                  }}>{field.label}</Typography><TextField id={field.name} name={field.name} type={field.type || "text"} value={formData[field.name as keyof typeof formData]} onChange={handleInputChange} disabled={loading} required={field.required} placeholder={field.placeholder} fullWidth sx={inputStyle} /></Grid>)}
                <Grid size={12}><Typography
                  ...
                  sx={{
                    ...
                    mb: 0.75,
                    display: "block"
                  }}>What would you like to build?</Typography><TextField id="projectDescription" name="projectDescription" value={formData.projectDescription} onChange={handleInputChange} disabled={loading} required placeholder="Tell us about your idea, audience, preferred timeline or anything else we should know." fullWidth multiline rows={5} sx={inputStyle} /></Grid>
              </Grid>
            </Box>
            <Stack
              ...
              sx={{
                alignItems: "center",
                color: "#526573"
              }}><FiClock /><Typography variant="body2">We typically reply within 1–2 business days.</Typography></Stack><Button type="submit" disabled={loading} variant="contained" size="large" endIcon={loading ? <CircularProgress color="inherit" size={18} /> : <FiSend />} sx={{ px: 3 }}>Send project enquiry</Button></Stack>
          </Stack>
          <Stack
            direction={{ xs: "column", sm: "row" }}
            spacing={1}
